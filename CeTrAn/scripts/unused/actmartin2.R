...
c.activity_martin <- function(traj, min_pause_length=0) {
...
D=c(D,disttrav1s)		
		...
		moving =ifelse (disttrav1s >2, TRUE,movprec) 
...
f_table = data.frame (f_table,sum_act_martin=sum_act,median_act_martin=median_act,
median_pause_martin=median_pause,number_pause_martin=number_pause, median_lin_martin=median_lin)
...
mybarplot(mean_table$means$sum_act,mean_table$ses$sum_act,rownames(mean_table$means),
	main="Total activity time Martin",ylab="Total activity time [s]", ylim= c(0,ylim_acttime))
mybarplot(mean_table$means$median_act,mean_table$ses$median_act,rownames(mean_table$means),
	main="Mean of medians of bouts length Martin",ylab="median of bouts length [s]",ylim = c(0,20))

mybarplot(mean_table$means$number_pause,mean_table$ses$number_pause,rownames(mean_table$means),
	main="Number of pauses Martin",ylab="Number of pauses", ylim = c(0,ylim_pauses))
...
mybarplot(mean_table$means$median_lin,mean_table$ses$median_lin,rownames(mean_table$means),
	main="Mean of medians of linearity score Martin",ylab="Lin score [0 to 1]", ylim =c(0, 1))
