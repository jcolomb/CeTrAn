...
# 
# PCA_res= data.frame(scores)
# PCA_res$group = i_table[,1]
annovatable = cbind(PCA_res, f_table [,c(3:7)])

x1= aov(annovatable$PC1~annovatable$genotype * annovatable$other)
#summary(x)
x2= aov(annovatable$PC2~annovatable$genotype * annovatable$other)
#summary(x)
x3= aov(annovatable$PC3~annovatable$genotype * annovatable$other)
#summary(x)
ANOVA=cbind(x1,x2,x3)
pdf("anovas.pdf")
x=x1
summary (x)
TukeyHSD(x, which = annovatable$genotype)
plot(TukeyHSD(x, "annovatable$genotype"), las=2)
x=x2
summary (x)
TukeyHSD(x)
plot(TukeyHSD(x, "annovatable$genotype"), las=2)
